...
class Soil extends Component {  
  ...
  getSoilTable() {
    ...
    let soilCollection = soil.data;

    if (filterUser !== '') {
      soilCollection = soilCollection.filter(
    ...
    return soilCollection.map(item => [
      String(item.id),
      item.notes,
    ...
    return (
      <GridContainer>
        <GridItem xs={12} sm={12} md={12}>
          <SoilChart water={water} environment={environment} biodiversity={biodiversity} soil={soil} production={production} climate={climate} user={user} addFavourites={this.addFavourites} />
        </GridItem>
        ...
        <GridItem xs={12} sm={12} md={12}>
          <Card>
            <CardHeader color="primary">
              <h4 className={classes.cardTitleWhite}>Muestras de Tierra y Suelo</h4>
            ...
            <CardBody>
              <Table
                onClick={item => history.push(`/soil/${item[0]}`)}
                tableHeaderColor="primary"
                tableHead={['ID', 'Notas', 'Provincia', 'Ciudad', 'Usuario', 'Fecha']}
                tableData={this.getSoilTable()}
              />
            </CardBody>
          </Card>
        </GridItem>
...
Soil.propTypes = {
  ...
  fetchSoil: PropTypes.func.isRequired,
